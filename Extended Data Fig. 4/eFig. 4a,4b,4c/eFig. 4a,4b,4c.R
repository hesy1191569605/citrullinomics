...
dat=data%>%filter(Group=="RA_A"|Group=="RA_B",`Drugs-HM`=="MTXLEF")
...
ggsave("eFig 4a.pdf", width = 3.5, height = 3)
...
ggsave("eFig 4b.pdf", width = 3.5, height = 3)
...
ggplot(dat1, aes(variable, value, color=`Responder-HM`, fill=`Responder-HM`))+
  ...
  theme(axis.text = element_text(color="black",size=15),
        ...
        axis.ticks =  element_line(color="black"),
        axis.title = element_text(color="black",size=15))
ggsave("eFig 4c.pdf", width = 4.5, height = 3)