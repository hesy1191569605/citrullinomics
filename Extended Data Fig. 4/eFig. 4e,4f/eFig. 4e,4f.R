...
dat1 = data.table::fread("eFig 4e,4f.csv")
...
for(i in 1:100){
  ...
  x <- cbind(train[,c("SJC","CRP","TJC","DAS28-CRP-HM")])
  x_test <- cbind(test[,c("SJC","CRP","TJC","DAS28-CRP-HM")])
...
ggsave("eFig 4e (left).pdf", width = 3, height = 3.3)
...
plot_ROC(list_train,show.AUC = F,show.points = F,
  ...
  annotate("text",x=0.65,y=0.2, label="AUROC range: 0.80-0.99", size=4)+
  annotate("text",x=0.65,y=0.1, label="Median AUROC: 0.93", size=4)

ggsave("eFig 4e (middle).pdf", width = 3.2, height = 3)
...
plot_ROC(list_test,show.AUC = F,show.points = F,
  ...
  theme(axis.text = element_text(color="black", size=13),
        axis.title = element_text(color="black", size=13),
        panel.grid = element_blank())+
  annotate("text",x=0.65,y=0.2, label="AUROC range: 0.81-0.99", size=4)+
  annotate("text",x=0.65,y=0.1, label="Median AUROC: 0.89", size=4)
ggsave("eFig 4e (right).pdf", width = 3.2, height = 3)
...
coef_all$ID=ifelse(coef_all$ID=="DAS28-CRP-HM","A DAS28",
                   ifelse(coef_all$ID=="TJC","B TJC",
                          ifelse(coef_all$ID=="SJC","C SJC",
                                 ifelse(coef_all$ID=="CRP","D CRP",NA))))
...
ggplot(coef_all, aes(x = coef, y = ID)) +
  ...
  theme(axis.line = element_line(color="black"),
        ...
        axis.title = element_text(color="black", size=13),
        legend.position = "bottom")
ggsave("eFig 4f.pdf", width = 6, height = 5)
