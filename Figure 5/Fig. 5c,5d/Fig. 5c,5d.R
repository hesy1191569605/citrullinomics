...
rm(list = ls())
dat1 = data.table::fread("Fig 5c,5d.csv")
...
for(i in 1:100){
  ...
  x <- cbind(train[,c("TJC","DAS28-CRP-HM")],train[,10:ncol(train)])
  x_test <- cbind(test[,c("TJC","DAS28-CRP-HM")],test[,10:ncol(test)])
...
ggsave("Fig 5a (left).pdf",  width = 3, height = 3.4)
...
plot_ROC(list_train,show.AUC = F,show.points = F,
  ...
  annotate("text",x=0.65,y=0.2, label="AUROC range: 0.72-1.00", size=4)+
  annotate("text",x=0.65,y=0.1, label="Median AUROC: 0.89", size=4)
...
plot_ROC(list_test,show.AUC = F,show.points = F,
  ...
  theme(axis.text = element_text(color="black", size=13),
        axis.title = element_text(color="black", size=13),
        panel.grid = element_blank())+
  annotate("text",x=0.65,y=0.2, label="AUROC range: 0.59-0.98", size=4)+
  annotate("text",x=0.65,y=0.1, label="Median AUROC: 0.80", size=4)
ggsave("Fig 5a (right).pdf", width = 3.2, height = 3)


coef_all$ID=ifelse(coef_all$ID=="P05023_R(pa)AVAGDASESALLK_","A P05023_R(pa)AVAGDASESALLK_",
                   ifelse(coef_all$ID=="TJC","B TJC",
                          ifelse(coef_all$ID=="DAS28-CRP-HM","C DAS28",
                                 ifelse(coef_all$ID=="O43707_AETAANR(pa)ICK_","D O43707_AETAANR(pa)ICK_",
                                        ifelse(coef_all$ID=="P25705_R(pa)LTDADAMK_","E P25705_R(pa)LTDADAMK_",
                                               ifelse(coef_all$ID=="P35749_VGR(pa)DVVQK_","F P35749_VGR(pa)DVVQK_",NA))))))
